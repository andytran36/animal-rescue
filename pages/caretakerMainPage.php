...
include_once '../includes/connectToDB.php';
	...
	$sql = "SELECT * 
			FROM rescue.member as m, rescue.caretaker as c
			WHERE m.member_id='".$member_id."'AND m.password='".$pass."' AND m.member_id=c.member_id;";
	...
	while($row = mysqli_fetch_assoc($result)){
				...
				$trained_species = $row['trained_species'];
	}
?>
...
<body>
	...
	<div class="welcome"><h1 class="welcomeTxt">WELCOME CARETAKER</h1></div>

	<div class="caretakerInfo jumbotron">
		<h4 class="textCenter">PERSONAL INFORMATION:</h4>
		<div class="row">
			<div class="col card">
				<h5>First Name: <?php echo $fname; ?></h5>
				<h5>Middle Initial(s): <?php echo $minit; ?></h5>
			...
			<div class="col card">
				...
				<h5>Trained Species: <?php echo $trained_species; ?></h5>
			</div>
		</div>
	</div>

	<div class="jumbotron container">
		<h4 class="textCenter">ADD NEW CARES ENTRY:</h4>
		<form method="POST" action="../includes/newCares.php">
	...
	<div class="jumbotron container">
		<h4 class="textCenter">REMOVE A CARES ENTRY:</h4>
		<form method="POST" action="../includes/removeCares.php">
			<input type="hidden" name="member_id" value=<?php echo "'" + $member_id + "'"?>>
			<div class="row">
				<div class="col">
					<label>Animal ID: </label>
			...
			if(mysqli_num_rows($result) > 0){
				...
				<table>
					<tr>
						...
						<th style='width:100px'>CARE DATE         </th>
						<th style='width:200px'>CARE DURATION     </th>
					</tr>";
				while($row = mysqli_fetch_assoc($result)){
					echo "<tr><td>". $row["animal_id"] ."</td><td>". $row["care_date"] ."</td><td>". $row["care_duration"] . " hours" ."</td><td>";
				}
				echo "</table>";
			}
			else{
	...
	<div class="jumbotron container">
		...
		<?php
			...
			if(mysqli_num_rows($result) > 0){
				...
				while($row = mysqli_fetch_assoc($result)){
					echo "<tr><td>". $row["event_id"] ."</td><td>". $row["event_type"] ."</td><td>". $row["event_location"]."</td><td>". $row["role"]."</td><td>";
				}
				echo "</table>";
			}
			else{
